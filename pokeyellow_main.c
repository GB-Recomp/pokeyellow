...
static const GBGameAssets GB_POKEYELLOW_GAME = {
    .game_id = "pokeyellow",
    .rom_filename = "roms/pokeyellow.gbc",
    .rom_data = pokeyellow__rom_data,
    .rom_size = 1048576u,
    .expected_sha1 = { 0xcc, 0x7d, 0x03, 0x26, 0x2e, 0xbf, 0xaf, 0x2f, 0x06, 0x77, 0x2c, 0x1a, 0x48, 0x0c, 0x7d, 0x9d, 0x5f, 0x4a, 0x38, 0xe1 },
    .manifest = POKEYELLOW_ASSETS_MANIFEST,
    .manifest_count = POKEYELLOW_ASSETS_MANIFEST_COUNT,
};
...
int pokeyellow_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *pokeyellow_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "pokeyellow");
        gb_context_set_save_id(interpreted_ctx, "pokeyellow");
        pokeyellow_init(generated_ctx);
        pokeyellow_init(interpreted_ctx);
    ...
    gb_context_set_save_id(ctx, "pokeyellow");
    ...
    if (!gb_load_assets(&GB_POKEYELLOW_GAME)) {
        fprintf(stderr, "[pokeyellow] failed to load ROM/assets\n");
        return 1;
    }
#ifdef GB_HAS_SDL2
    gb_platform_set_game_id(ctx, GB_POKEYELLOW_GAME.game_id);
#endif
    pokeyellow_init(ctx);
    int exit_code = 0;
